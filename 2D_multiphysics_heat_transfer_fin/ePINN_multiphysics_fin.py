...
def geo_train(device,x_influid,y_influid,x_infin,y_infin,xd,yd,Td,xb,yb,xleft,yleft,xbase_fin,ybase_fin,xb_wall,yb_wall,xb_wall_fluid,yb_wall_fluid,xb_wall_fin, yb_wall_fin,xup_fluid, yup_fluid, xleft_fluid, yleft_fluid, xright_fluid, yright_fluid, xup_fin, yup_fin, xleft_fin, yleft_fin, xright_fin, yright_fin,ub,vb,Tb,batchsize,learning_rate,epochs,path,Flag_batch,Diff,rho,Lambda_BC,nPt ):
	...
	class Net2_kc(nn.Module):  
		...
		def __init__(self):
			...
			self.final = nn.Sequential(
				nn.Linear(10,40),         # take outputs as input to main network
				Swish(),
				nn.Linear(40,40),
				Swish(),
				nn.Linear(40,40),
	...
	# read patterns for initializations
	if (Flag_pretrain):
	...
	if (Flag_pretrain_initialization):
		net2_kc.k1.load_state_dict(torch.load(path+"heat_conductivity_1_200"+".pt"))
		net2_kc.k2.load_state_dict(torch.load(path+"heat_conductivity_2_200"+".pt"))
		net2_kc.k3.load_state_dict(torch.load(path+"heat_conductivity_3_200"+".pt"))
		net2_kc.k4.load_state_dict(torch.load(path+"heat_conductivity_4_200"+".pt"))
		net2_kc.k5.load_state_dict(torch.load(path+"heat_conductivity_5_200"+".pt"))
		net2_kc.k6.load_state_dict(torch.load(path+"heat_conductivity_6_200"+".pt"))
		net2_kc.k7.load_state_dict(torch.load(path+"heat_conductivity_7_200"+".pt"))
		net2_kc.k8.load_state_dict(torch.load(path+"heat_conductivity_8_200"+".pt"))
		net2_kc.k9.load_state_dict(torch.load(path+"heat_conductivity_9_200"+".pt"))
		net2_kc.k10.load_state_dict(torch.load(path+"heat_conductivity_10_200"+".pt"))
	...
	if (Flag_Freezing_layer):
		...
		for parameter_1 in net2_kc.k1.main.parameters():
			 parameter_1.requires_grad = False            
			   
		for parameter_2 in net2_kc.k2.main.parameters():
			 parameter_2.requires_grad = False     

		for parameter_3 in net2_kc.k3.main.parameters():
			 parameter_3.requires_grad = False         

		for parameter_4 in net2_kc.k4.main.parameters():
			 parameter_4.requires_grad = False   

		for parameter_5 in net2_kc.k5.main.parameters():
			 parameter_5.requires_grad = False  

		for parameter_6 in net2_kc.k6.main.parameters():
			 parameter_6.requires_grad = False 

		for parameter_7 in net2_kc.k7.main.parameters():
			 parameter_7.requires_grad = False              
				  
		for parameter_8 in net2_kc.k8.main.parameters():
			 parameter_8.requires_grad = False    

		for parameter_9 in net2_kc.k9.main.parameters():
			 parameter_9.requires_grad = False  
			 
		for parameter_10 in net2_kc.k10.main.parameters():
	...
	if(Flag_batch):# This one uses dataloader
			...
			for epoch in range(epochs):
				...
				if epoch % 1000 == 0:#save network
				 torch.save(net2_u.state_dict(),path+"inv_step_u_parallel_ten_class_"+str(epoch)+".pt")
				 torch.save(net2_v.state_dict(),path+"inv_step_v_parallel_ten_class_"+str(epoch)+".pt")
				 torch.save(net2_p.state_dict(),path+"inv_step_p_parallel_ten_class_"+str(epoch)+".pt")
				 torch.save(net2_T.state_dict(),path+"inv_step_T_parallel_ten_class_"+str(epoch)+".pt")
				 torch.save(net2_Tc.state_dict(),path+"inv_step_Tc_parallel_ten_class_"+str(epoch)+".pt")
				 torch.save(net2_kc.state_dict(),path+"inv_step_kc_parallel_ten_class_"+str(epoch)+".pt")
...
Flag_pretrain = False # do pretraining 
