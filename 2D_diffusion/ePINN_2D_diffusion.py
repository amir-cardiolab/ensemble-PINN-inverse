...
def geo_train(device,xfin,yfin,xd,yd,Td,xbase_fin,ybase_fin,xb_wall_fin, yb_wall_fin, xup_fin, yup_fin, xleft_fin, yleft_fin, xright_fin, yright_fin,Tb_up,Tb_down,batchsize,learning_rate,epochs,path,Flag_batch,Lambda_BC,nPt ):
	...
	class Net2_kc(nn.Module):  
		...
		def __init__(self):
			...
			self.final = nn.Sequential(
				nn.Linear(10,40),         # outputs are taken as inputs
				Swish(),
				nn.Linear(40,40),
				Swish(),
				nn.Linear(40,40),
	...
	if (Flag_pretrain_initialization):
		...
		net2_kc.k1.load_state_dict(torch.load(path+"heat_conductivity_diffusion_1_200"+".pt"))
		net2_kc.k2.load_state_dict(torch.load(path+"heat_conductivity_diffusion_2_200"+".pt"))
		net2_kc.k3.load_state_dict(torch.load(path+"heat_conductivity_diffusion_3_200"+".pt"))
		net2_kc.k4.load_state_dict(torch.load(path+"heat_conductivity_diffusion_4_200"+".pt"))
		net2_kc.k5.load_state_dict(torch.load(path+"heat_conductivity_diffusion_5_200"+".pt"))
		net2_kc.k6.load_state_dict(torch.load(path+"heat_conductivity_diffusion_6_200"+".pt"))
		net2_kc.k7.load_state_dict(torch.load(path+"heat_conductivity_diffusion_7_200"+".pt"))
		net2_kc.k8.load_state_dict(torch.load(path+"heat_conductivity_diffusion_8_200"+".pt"))
		net2_kc.k9.load_state_dict(torch.load(path+"heat_conductivity_diffusion_9_200"+".pt"))
		net2_kc.k10.load_state_dict(torch.load(path+"heat_conductivity_diffusion_10_200"+".pt"))
	...
	if(Flag_batch):# This one uses dataloader
			...
			for epoch in range(epochs):
				...
				if epoch % 1000 == 0:#save network
				 ...
				 torch.save(net2_Tc.state_dict(),path+"parallel_Tc_pure_conduction_ten_class_"+str(epoch)+".pt")
				 torch.save(net2_kc.state_dict(),path+"parallel_kc_pure_conduction_ten_class_"+str(epoch)+".pt")
